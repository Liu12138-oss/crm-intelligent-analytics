...
function Extract-PlanField {
    ...
    $regex = "^\*\*$([Regex]::Escape($FieldPattern))\*\*[:\uFF1A]\s*(.+)$"
...
function Extract-FirstPlanField {
    param(
        [Parameter(Mandatory=$true)]
    ...
    foreach ($fieldPattern in $FieldPatterns) {
        $value = Extract-PlanField -FieldPattern $fieldPattern -PlanFile $PlanFile
...
function Parse-PlanData {
    ...
    $script:NEW_LANG         = Extract-FirstPlanField -FieldPatterns @('Language/Version','语言/版本') -PlanFile $PlanFile
    $script:NEW_FRAMEWORK    = Extract-FirstPlanField -FieldPatterns @('Primary Dependencies','主要依赖') -PlanFile $PlanFile
    $script:NEW_DB           = Extract-FirstPlanField -FieldPatterns @('Storage','存储') -PlanFile $PlanFile
    $script:NEW_PROJECT_TYPE = Extract-FirstPlanField -FieldPatterns @('Project Type','项目类型') -PlanFile $PlanFile

    if ($script:NEW_LANG) { Write-Info "Found language: $script:NEW_LANG" } else { Write-WarningMsg 'No language information found in plan' }
    if ($script:NEW_FRAMEWORK) { Write-Info "Found framework: $script:NEW_FRAMEWORK" }
    if ($script:NEW_DB -and $script:NEW_DB -ne 'N/A') { Write-Info "Found database: $script:NEW_DB" }
    if ($script:NEW_PROJECT_TYPE) { Write-Info "Found project type: $script:NEW_PROJECT_TYPE" }
    return $true
}
...
function New-AgentFile {
    ...
    $escaped_lang = $script:NEW_LANG
    $escaped_framework = $script:NEW_FRAMEWORK
...
function Update-ExistingAgentFile {
    ...
    if ($script:NEW_DB -and $script:NEW_DB -notin @('N/A','NEEDS CLARIFICATION')) {
        $escapedDB = [Regex]::Escape($script:NEW_DB)
    ...
    elseif ($script:NEW_DB -and $script:NEW_DB -notin @('N/A','NEEDS CLARIFICATION')) { $newChangeEntry = "- ${CURRENT_BRANCH}: Added ${script:NEW_DB}" }
...
function Print-Summary {
    ...
    if ($script:NEW_LANG) { Write-Host "  - Added language: $script:NEW_LANG" }
    if ($script:NEW_FRAMEWORK) { Write-Host "  - Added framework: $script:NEW_FRAMEWORK" }
    if ($script:NEW_DB -and $script:NEW_DB -ne 'N/A') { Write-Host "  - Added database: $script:NEW_DB" }
    Write-Host ''
