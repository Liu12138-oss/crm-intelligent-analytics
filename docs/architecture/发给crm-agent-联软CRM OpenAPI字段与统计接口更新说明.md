...
本轮面向 AI-agent 数据仓库语义层和 Text-to-SQL 又补充了以下稳定口径：
...
| 报价区域字段 | `quotes.region / quotes.bigRegion` 支持按“报价自身 -> 关联商机 -> 渠道 -> 负责人”继承。 |
| 订单区域字段 | `orders.region / orders.bigRegion` 支持按“订单自身 -> 关联报价 -> 关联商机 -> 渠道 -> 负责人”继承。 |
| 权限过滤 | 区管查询报价/订单时使用标准化后的 `region` 裁剪，避免原始报价/订单区域为空导致漏数。 |
| 关系与指标 | 已补充客户、服务商、报备、商机、报价、订单主外键关系、匹配规则和指标口径。 |
| 样例回归 | 已按超管、区管、渠道、销售员工四类账号补充样例问题和期望权限行为。 |

详细字段、关系、指标、字典和样例请见：

```text
docs/发给crm-agent-联软CRM权限字段关系指标字典样例补充回复.md
```
...
## 11. 2026-06-08 角色权限与企微映射补充

依据 AI-agent 新发《角色权限与企微映射字段补充清单》，本轮 OpenAPI 输出层继续补齐以下内容。

### 11.1 用户与企微映射字段

`GET /users`、`GET /users/{id}`、`GET /auth/me`、`GET /identity/users/{userId}` 均稳定返回或兼容返回以下字段：

| 字段 | 说明 |
|---|---|
| `wecomUserId` | 企业微信 userid；当前未维护映射时返回空字符串。 |
| `mobile` | 手机号兼容字段，当前由 `mobile || phone` 兜底。 |
| `email` | 邮箱。 |
| `roleName` | 单角色中文名。 |
| `roleIds` | 角色编码数组，单角色时如 `["admin"]`。 |
| `roleNames` | 角色名称数组，单角色时如 `["区域管理员"]`。 |
| `isAdmin` | 是否管理类账号，当前 `superadmin/admin` 为 `true`。 |
| `departmentId` / `departmentIds` | 部门 ID 和部门 ID 数组；无组织映射时为空。 |
| `organizationId` / `organizationIds` | 组织 ID 和组织 ID 数组；无组织映射时为空。 |
| `managedUserIds` | 当前 CRM 用户权限内可见用户 ID。 |
| `ownerIds` | 当前 CRM 用户可见负责人 ID，第一阶段与 `managedUserIds` 同步。 |
...
`/users/{userId}/permission-scope` 用于查询指定 CRM 用户的权限范围。调用方必须具备 `users` 资源授权，且只能查询当前 token 绑定用户可见范围内的用户。

`/meta/role-permissions` 返回四类角色的数据范围矩阵、对象过滤规则和二次裁剪依赖字段，便于 AI-agent 建立本地权限口径。

### 11.3 六类对象稳定归属字段

以下字段已在对象输出层尽量补齐；无真实值时返回空字符串，字段名保持稳定：

| 对象 | 稳定归属字段 |
|---|---|
| `partners` | `ownerId`、`ownerName`、`partnerId/id`、`region`、`bigRegion`、`departmentId`、`organizationId`、`parentPartnerId`、`parentPartnerIds` |
...
5. 分别拉取 `partners/registrations/opportunities/quotes/orders` 各 1 条，确认归属字段是否稳定出现。

## 12. 2026-06-09 智能分析全量补充

本轮按对方《智能分析 OpenAPI 全量补充清单》补充客户主数据视图和客户分析接口。

### 12.1 新增客户接口

```http
GET /api/open/v1/customers
GET /api/open/v1/customers/{id}
GET /api/open/v1/customers/{id}/registrations
GET /api/open/v1/customers/{id}/opportunities
GET /api/open/v1/customers/{id}/quotes
GET /api/open/v1/customers/{id}/orders
```

说明：当前 `customers` 是只读兼容视图，由报备、商机、报价、订单归并生成；不写业务表，不影响 CRM 页面。
...
GET /api/open/v1/analytics/customers/idle?idleDays=90
GET /api/open/v1/analytics/customers/summary
```
...
`registrations/opportunities/quotes/orders` 已稳定返回：
...
当前 `customerIdRule` 包括：

| 值 | 说明 |
|---|---|
| `customerId` | 使用真实客户 ID。 |
